...
public class EchartsController {
    ...
    Result<List<EchartsData>> getEchartsData(@PathVariable String modelName) {
		//接收一个modelName参数
		//根据不同的modelName调用不同的Service的findAll方法获取对应的数据列表
		//然后根据数据列表生成对应的饼图和柱状图数据，最后将数据封装为EchartsData列表并返回
    ...
    @GetMapping("/options")
		...
		//创建一个空的选项列表options
		//然后逐个创建Map对象，设置value和label，最后将Map对象添加到选项列表中，并返回
    ...
    private void getPieData(String name, List<EchartsData> pieList, Map<String, Integer> dataMap) {
		//根据传入的name和dataMap生成对应的饼图数据，并将数据封装为EchartsData对象添加到pieList中
        EchartsData pieData = new EchartsData();
        EchartsData.Series series = new EchartsData.Series();

		//创建EchartsData对象和Series对象
		//设置饼图的标题、名称、类型、半径等属性，然后遍历dataMap
		//将数据添加到饼图的数据列表中，最后将生成的饼图数据添加到pieList中
    ...
    private void getBarData(String name, List<EchartsData> barList, Map<String, Integer> dataMap) {
		//根据传入的name和dataMap生成对应的柱状图数据，并将数据封装为EchartsData对象添加到barList中
		// 创建EchartsData对象和Series对象，
		// 设置柱状图的数据、x轴、y轴、图例、提示框等属性，
		// 然后遍历dataMap，将数据添加到柱状图的数据列表中，最后将生成的柱状图数据添加到barList中
